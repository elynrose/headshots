...
@can('generate_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.generates.create') }}">
                ...
                <tbody>
                    @foreach($generates as $key => $generate)
                        <tr data-entry-id="{{ $generate->id }}">
                            ...
                            <td>
                                @can('generate_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.generates.show', $generate->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('generate_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.generates.edit', $generate->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('generate_delete')
                                    <form action="{{ route('admin.generates.destroy', $generate->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('generate_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.generates.massDestroy') }}",

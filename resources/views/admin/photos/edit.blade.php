...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.photos.update", [$photo->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('use_for_training') ? 'is-invalid' : '' }}">
                    <input type="hidden" name="use_for_training" value="0">
                    <input class="form-check-input" type="checkbox" name="use_for_training" id="use_for_training" value="1" {{ $photo->use_for_training || old('use_for_training', 0) === 1 ? 'checked' : '' }}>
                    <label class="form-check-label" for="use_for_training">{{ trans('cruds.photo.fields.use_for_training') }}</label>
                </div>

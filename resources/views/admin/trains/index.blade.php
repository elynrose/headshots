...
@can('train_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.trains.create') }}">
                ...
                <tbody>
                    @foreach($trains as $key => $train)
                        <tr data-entry-id="{{ $train->id }}">
                            ...
                            <td>
                                @can('train_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.trains.show', $train->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('train_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.trains.edit', $train->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('train_delete')
                                    <form action="{{ route('admin.trains.destroy', $train->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('train_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.trains.massDestroy') }}",
